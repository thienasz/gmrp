...
//    $games = \App\Models\Game::all()->pluck('id')->toArray();
    $data = \App\Models\GameAgency::all();
    $index = $faker->numberBetween(0, $data->count() - 1);
    $user = $data[$index];
    ...
    return [
        ...
        'role' => $faker->randomElement(['0', '2']),
        'device_uid' => $faker->uuid,
        'os_type' => $faker->randomElement(['Android', 'Ios']),
        'os_version' => "1.1",
    ];
});
...
$factory->define(App\Models\PaymentType::class, function (Faker\Generator $faker){
    return [
        'name' => $faker->unique()->randomElement(['Viettel', 'Mobi', 'Vina']),
    ];
});
...
$factory->define(App\Models\GameAgency::class, function (Faker\Generator $faker) {
    $games = \App\Models\Game::all()->pluck('id')->toArray();
